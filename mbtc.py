...
group.add_argument('-t', action='store_const', const='ticker',              help='get account info')
...
if op['t'] != None: 
    ...
    try:
        ...
        H.update(REQUEST_PATH + '?' + params)
        headers = { 'Content-type': 'application/x-www-form-urlencoded', 'TAPI-ID': MB_TAPI_ID, 'TAPI-MAC': H.hexdigest() }
        conn.request('POST', REQUEST_PATH, params, headers)
        response = conn.getresponse()
        response_json = json.loads(response.read(), object_pairs_hook=OrderedDict)
